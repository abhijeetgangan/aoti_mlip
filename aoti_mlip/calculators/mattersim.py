...
from ase.constraints import full_3x3_to_voigt_6_stress  # ty: ignore[unresolved-import]
...
class MatterSimCalculator(Calculator):
    ...
    def calculate(  # type: ignore
        ...
        graph_batch = next(iter(dataloader)).to(self.device)
        results = self.model(*unpack_graph_batch(graph_batch))
        self.results.update(
